...
|---|---|---|
| `src/Design System/tokens.json`, `tokens.css` and the two `tokens.primitives.*` | `variables.figma.json` and `typography.figma.json` (Figma exports) | `npm run tokens:build` |
...
- **Consume the semantic layer, never a primitive.** `tokens.css` holds the names
  a component may use; the `--ds-brand-*` and `--ds-alias-*` names in
  `tokens.primitives.css` exist only for those to point at. A test enforces this.
